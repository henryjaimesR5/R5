...
class TestHttpGetIntegration:
    ...
    async def test_get_with_query_params(self, http_client):
        ...
        result = await http_client.get(
            "https://jsonplaceholder.typicode.com/posts", params={"userId": 1}
        )
...
class TestHttpPostIntegration:
    ...
    async def test_post_form_data(self, http_client):
        ...
        form_data = {"title": "Form Test", "body": "Body text", "userId": 1}

        result = await http_client.post(
            "https://jsonplaceholder.typicode.com/posts", data=form_data
        )
...
class TestHttpPatchIntegration:
    ...
    async def test_patch_partial_update(self, http_client):
        ...
        result = await http_client.patch(
            "https://jsonplaceholder.typicode.com/posts/1", json=partial_update
        )
...
class TestHttpHandlers:
    ...
    async def test_on_status_handler(self, http_client):
        ...
        user = (
            result.on_status(200, lambda req, res: handler_calls.append("200_OK"))
    ...
    async def test_chaining_handlers(self, http_client):
        ...
        user = (
            result.on_status(404, lambda req, res: calls.append("not_found"))
...
class TestHttpTimeouts:
    ...
    async def test_custom_timeout(self, http_client):
        ...
        result = await http_client.get(
            "https://jsonplaceholder.typicode.com/users/1", timeout=15.0
        )
    ...
    async def test_timeout_on_slow_endpoint(self, http_client):
        ...
        result = await http_client.get("https://httpbin.org/delay/10", timeout=2.0)
...
class TestHttpHeaders:
    ...
    async def test_custom_headers(self, http_client):
        ...
        custom_headers = {"Accept": "application/json", "X-Custom-Header": "R5-Test"}

        result = await http_client.get(
            "https://httpbin.org/headers", headers=custom_headers
        )
    ...
    async def test_user_agent_header(self, http_client):
        ...
        assert data is not None
        assert "R5-HTTP" in data.get("user-agent", "")
