        ...
        async with http_client:
            with patch.object(
                ...
                result = await http_client.put(
                    "https://api.example.com/test/1", json={"data": "updated"}
                )
        ...
        async with http_client:
            with patch.object(
                ...
                result = await http_client.patch(
                    "https://api.example.com/test/1", json={"data": "patched"}
                )
    ...
    async def test_request_with_exception(self, http_client):
        ...
        async with http_client:
            with patch.object(
                ...
                from R5.http.errors import HttpTimeoutError

                assert isinstance(result.exception, HttpTimeoutError)
                assert result.status == 0
        ...
        async with http_client:
            with patch.object(
                ...
                result = await http_client.get(
                    "https://api.example.com/test", timeout=10.0
                )
    ...
    async def test_custom_headers(self, http_client):
        ...
        async with http_client:
            with patch.object(
                ...
                result = await http_client.get(
                    "https://api.example.com/test", headers=custom_headers
                )
...
class TestHttpConfig:
    ...
    def test_default_config(self):
        ...
        assert config.http_user_agent == "R5-HTTP/1.0"
...
class TestInterceptors:
    ...
    async def test_interceptors_chaining(self):
        ...
        result = (
            http.on_before(lambda req: None)
        ...
        async with http:
            ...
            with patch.object(http, "_ensure_client") as mock_client_getter:
                ...
                mock_client.build_request = Mock(
                    side_effect=lambda *args, **kwargs: request_200
                )
                ...
                assert (
                    len(call_count) == 2
                )  # Primera falló (503), segunda exitosa (200)
    ...
    async def test_on_status_vs_when_status(self):
        ...
        async with http:
            ...
            result = await http.get(
                ...
                on_status={404: lambda: on_status_called.append("handler_404")},
            )
    ...
    async def test_combined_when_and_on_handlers(self):
        ...
        async with http:
            with patch.object(http, "_ensure_client") as mock_client_getter:
                ...
                assert (
                    events.count("before") == 2
                )  # on_before se ejecuta en cada intento
...
class TestResultNullValidation:
    ...
    def test_optional_field_with_none_no_warning(self):
        ...
        mock_response = Mock(spec=httpx.Response)
        mock_response.json.return_value = {"id": 1, "name": "John", "email": None}
    ...
    def test_dataclass_mapping_with_mixed_null_values(self):
        ...
        mock_response = Mock(spec=httpx.Response)
        mock_response.json.return_value = {"id": 100, "name": "Alice", "email": None}
